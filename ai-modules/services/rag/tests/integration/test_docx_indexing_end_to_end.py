...
from tests._docx_fixtures import make_docx_with_headings, make_empty_docx, make_text_docx
...
def milvus_store() -> Generator[MilvusVectorStore]:
    ...
    store = MilvusVectorStore(
        host=host,
        port=port,
        collection_name=f"verify_docx_indexing_{uuid.uuid4().hex[:8]}",
        vector_dim=1024,
    )
    yield store
    try:
...
async def test_docx_indexing_publishes_chunks_with_headings(
    embedder: BgeM3Embedder,
    milvus_store: MilvusVectorStore,
    chunker: ParagraphChunker,
    document_parser: FormatDispatchingDocumentParser,
) -> None:
    """DOCX 文档入库后，Milvus 中应能观察到包含中文内容的片段。"""
    content = make_docx_with_headings(
        [
            (1, "知识库管理办法", ["本办法用于规范公司知识管理。"]),
        ]
    )
    document_id = f"doc-docx-{uuid.uuid4().hex[:8]}"
    ...
    command = IndexingCommand(
        ...
        source_url="http://minio/regulations.docx",
    ...
    assert "本办法用于规范公司知识管理" in full_text

    # 03e：通过 search 验证来源元数据在召回结果中可观察
    # DOCX 解析器为片段填充 section_title，page_number 始终为 None
...
@pytest.mark.asyncio
@pytest.mark.skipif(not os.environ.get("SAGE_VAULT_RAG_RUN_MILVUS_TESTS"), reason="需要显式启用 Milvus 集成测试")
async def test_docx_indexing_enables_recall_in_qa(
    embedder: BgeM3Embedder,
    milvus_store: MilvusVectorStore,
    chunker: ParagraphChunker,
    document_parser: FormatDispatchingDocumentParser,
    ...
    document_id = f"doc-docx-qa-{uuid.uuid4().hex[:8]}"
    knowledge_base_id = 110
    ...
    command = IndexingCommand(
        ...
        knowledge_base_id=knowledge_base_id,
        document_id=document_id,
        filename="benefits.docx",
        source_url="http://minio/benefits.docx",
        request_id=f"req-{document_id}",
    )
    ...
    events = [event async for event in answering_service.answer(knowledge_base_id, "员工福利有哪些？", "gen-docx")]
...
@pytest.mark.asyncio
@pytest.mark.skipif(not os.environ.get("SAGE_VAULT_RAG_RUN_MILVUS_TESTS"), reason="需要显式启用 Milvus 集成测试")
async def test_empty_docx_indexing_returns_failure_without_writing_chunks(
    embedder: BgeM3Embedder,
    milvus_store: MilvusVectorStore,
    chunker: ParagraphChunker,
    document_parser: FormatDispatchingDocumentParser,
) -> None:
    """空白 DOCX 入库失败，不向 Milvus 写入任何片段，回调 FAILED 状态。"""
    content = make_empty_docx()
    document_id = f"doc-docx-empty-{uuid.uuid4().hex[:8]}"
    knowledge_base_id = 120
    ...
    command = IndexingCommand(
        ...
        document_id=document_id,
        filename="empty.docx",
        source_url="http://minio/empty.docx",
        request_id=f"req-{document_id}",
    )
    result = await service.index(command)
    ...
    assert result.diagnostics["filename"] == "empty.docx"
...
@pytest.mark.asyncio
@pytest.mark.skipif(not os.environ.get("SAGE_VAULT_RAG_RUN_MILVUS_TESTS"), reason="需要显式启用 Milvus 集成测试")
async def test_corrupted_docx_indexing_returns_failure_without_writing_chunks(
    embedder: BgeM3Embedder,
    milvus_store: MilvusVectorStore,
    chunker: ParagraphChunker,
    document_parser: FormatDispatchingDocumentParser,
) -> None:
    """损坏 DOCX 入库失败，不向 Milvus 写入任何片段，回调 FAILED 状态。"""
    content = b"This is not a valid DOCX file content"
    document_id = f"doc-docx-corrupted-{uuid.uuid4().hex[:8]}"
    knowledge_base_id = 130
    ...
    command = IndexingCommand(
        ...
        source_url="http://minio/corrupted.docx",
        request_id=f"req-{document_id}",
    )
    result = await service.index(command)
    ...
    assert result.chunks_count == 0
    assert result.diagnostics["error"] == "ValueError"
    assert result.diagnostics["filename"] == "corrupted.docx"
    assert await milvus_store.count_by_document(document_id) == 0
    assert callback.results == [result]
