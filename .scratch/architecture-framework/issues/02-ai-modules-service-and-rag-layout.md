...
采用 Python 3.12、FastAPI 和 LangChain，具体版本以架构规格的唯一版本表为准。FastAPI 只负责 HTTP/SSE transport，LangChain 只负责 RAG 编排实现；Java 保持业务状态和异步任务的唯一权威，Python 不引入 Celery 或业务数据库。
...
入库、回答和清理是三个深模块；transport 只转换协议。LangChain 和供应商 SDK 类型不得进入 HTTP schema、执行模型、port 或 Java-Python 契约。Milvus adapter 强制知识库过滤，百炼 adapter 转换为项目自有流事件。容器默认单 worker，通过有界执行器和 semaphore 控制并发。
