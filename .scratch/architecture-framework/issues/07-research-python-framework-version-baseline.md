...
- FastAPI 0.139.x、Pydantic 2.13.x 和 pydantic-settings 2.14.x 的声明范围相交；SSE 可直接使用 `StreamingResponse`。
...
- LangChain 评估后未采用：升级频繁，且 RAG 编排已由项目自有 application services 承担，避免引入额外抽象层。
