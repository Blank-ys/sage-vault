...
/**
 * 会话回答生命周期的深实现：收敛发起事务、SSE 事件流、停止信号、RAG 取消与终态裁决。
 * 持久化通过本能力 Mapper 与 {@code QaRecordService} 产生副作用，RAG 通过 port 接入。
 */
@Service
public class AnswerSessionServiceImpl implements AnswerSessionService {
    private static final Logger log = LoggerFactory.getLogger(AnswerSessionServiceImpl.class);
    ...
    public AnswerSessionServiceImpl(ConversationMapper mapper, KnowledgeBaseService knowledgeBases,
